...
public class ReportEmailService {
	...
	private static final String ISSUES_SUBJECT = "ISSUES Scan Report - %s Free : %s Full ";
	...
	public void checkAndSendReport(int days) {
		...
		SendEmailRequest request = new SendEmailRequest().withDestination(new Destination().withToAddresses(toList))
				.withMessage(new Message().withBody(new Body().withHtml(new Content().withCharset("UTF-8").withData(emailBody.toString())).withText(new Content().withCharset("UTF-8").withData("")))
						.withSubject(new Content().withCharset("UTF-8").withData(String.format(SUBJECT, freeList.size(), paidList.size()))))
				.withSource(FROM);
	...
	private void addPendingScansToBody(StringBuilder html, List<ScanReport> pendingList, String title, ScanType basicFolder, Properties properties) {

		if (pendingList.size() == 0)
			return;

		html.append("<h2>" + title + " - " + pendingList.size() + "</h2>");
		html.append("<table cellspacing=\"0\" cellpadding=\"2\" width=\"100%\"><tr><td><b>URL</b></td><td><b>Email</b></td><td><b>Status</b></td><td><b>Scan Time</b></td></tr>");
		int count = 0;
	...
	private void addScansToBody(StringBuilder html, List<ScanReport> reportList, String title, ScanType scanType, Properties properties) {
		...
		for (ScanReport report : reportList) {
			...
			if (report.getStatus() == -1)
				...
				html.append("<td><a href=\"" + ChannelGroupFactory.getChannelScanUrl(scanType, report, properties) + "\">Report</td>");
			...
			if (report.getStatus() == -1)
				...
				html.append("<td>Forbidden</td>");
			else if (report.getStatus() == 3 || (report.getTotalAIssues() == 0 && report.getTotalAAIssues() == 0))
				html.append("<td>No Issues</td>");
			else
	...
	private AmazonSimpleEmailService createSESClient() {
		AWSCredentials credentials = new BasicAWSCredentials(properties.getProperty(Property.AWS_S3_KEY), properties.getProperty(Property.AWS_S3_SECRET));
		AmazonSimpleEmailService client = AmazonSimpleEmailServiceClientBuilder.standard().withCredentials(new AWSStaticCredentialsProvider(credentials)).withRegion(Regions.US_WEST_2).build();

		return client;
	}

	public void checkAndSendIssuesReport(int minutes) {
		AmazonSimpleEmailService client = createSESClient();
		StringBuilder emailBody = new StringBuilder();
		...
		System.err.println(from.getTime());
		List<ScanReport> freeList = dao.getIssueScans("vch_basic_scan_id", "ss_basic_scan", "scopeFolder", from);
		List<ScanReport> paidList = dao.getIssueScans("vch_full_scan_id", "ss_full_scan", null, from);

		if (freeList.size() == 0 && paidList.size() == 0)
			return;

		emailBody.append(String.format("<h1>Period: Last %s minutes </h1>", minutes));
		addScansToBody(emailBody, freeList, "Free Scans", ScanType.BASIC_FOLDER, properties);
		addScansToBody(emailBody, paidList, "Full Scans", ScanType.FULL, properties);
		...
		List<String> toList = new ArrayList<>();
		String[] recipients = new String[] { "james@digipromedia.com" };
		for (String recipient : recipients)
			toList.add(recipient);

		SendEmailRequest request = new SendEmailRequest().withDestination(new Destination().withToAddresses(toList))
				.withMessage(new Message().withBody(new Body().withHtml(new Content().withCharset("UTF-8").withData(emailBody.toString())).withText(new Content().withCharset("UTF-8").withData("")))
						.withSubject(new Content().withCharset("UTF-8").withData(String.format(ISSUES_SUBJECT, freeList.size(), paidList.size()))))
				.withSource(FROM);
		// Comment or remove the next line if you are not using a
		// configuration set
		// .withConfigurationSetName(CONFIGSET);

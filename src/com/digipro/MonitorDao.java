...
public class MonitorDao extends BaseDao {
	...
	public List<ScanReport> getIssueScans(String idField, String tableName, String scanScope, Calendar from) {
		ResultSet rs = null;
		...
		try {
			...
			String sql = "SELECT * from " + tableName
					+ " where n_campaign_id = 0 and (n_status in(-1,-2,3) or (n_a_issue_count = 0 and n_aa_issue_count = 0)) and b_test = 0 and b_bulk_scan = 0 and d_created > ? ";
			...
			processResults(idField, reportList, rs);
	...
	private void processResults(String idField, List<ScanReport> reportList, ResultSet rs) throws Exception {
		while (rs.next()) {
	...
	public List<ScanReport> getScans(String idField, String tableName, String scanScope, Calendar from) {
		...
		try {
			...
			String sql = "SELECT * from " + tableName + " where n_campaign_id = 0 and n_status in(-1,2,3) and b_test = 0 and b_bulk_scan = 0 and d_created > ? ";
	...
	public List<ScanReport> getInProgressAndPendingFullScans() {
		...
		try {
			...
			String sql = "SELECT * from ss_full_scan " + " where n_campaign_id = 0 and n_status in(0,1) and b_test = 0 and b_bulk_scan = 0 ";
	...
	public LastAccess getLastAccess(String ipAddress, ScanType scanType) {
		...
		try {
			...
			stmt = conn.prepareStatement("SELECT * from ss_scan_monitor WHERE vch_scan_type = ? and vch_processor_ip = ? ");
